...
const ClipCard = ({ clip }) => {
    return (
        <div className="bg-white border rounded-xl overflow-hidden hover:shadow-xl transition duration-300 flex flex-col h-full group">
            <div className={`h-48 ${clip.color || 'bg-gray-300'} flex items-center justify-center relative`}>
                <Music className="w-12 h-12 text-white opacity-50 group-hover:scale-110 transition duration-500" />
            </div>
            
            <div className="p-5 flex flex-col flex-grow">
                <h3 className="font-bold text-lg text-gray-800 mb-1">{clip.song}</h3>
                ...
                <div className="mt-4 pt-2">
                    <PrimaryButton className="w-full text-sm">View Details</PrimaryButton>
                </div>
            </div>
        </div>
    );
